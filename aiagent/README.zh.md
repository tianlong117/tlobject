...
0. [特色与优势](#特色与优势)
   - [一、消息架构血统带来的结构性优势](#一消息架构血统带来的结构性优势)
   - [二、对话智能](#二对话智能)
   - [三、工具（Skill）能力](#三工具skill能力)
   - [四、三层记忆体系](#四三层记忆体系)
   - [五、多 Agent 编排](#五多-agent-编排)
   - [六、工程化与安全](#六工程化与安全)
1. [架构概览](#架构概览)
   - [设计原则](#设计原则)
2. [模块结构](#模块结构)
...
3. [核心类说明](#核心类说明)
   ...
   - [记忆实现](#记忆实现)
4. [数据POJO](#数据pojo)
   - [TLConversationHistory — 对话历史条目](#tlconversationhistory--对话历史条目)
   - [TLToolCall — 工具调用](#tltoolcall--工具调用)
   - [TLFunctionDefinition — 函数定义](#tlfunctiondefinition--函数定义)
   - [TLMemoryEntry — 记忆条目](#tlmemoryentry--记忆条目)
5. [消息流](#消息流)
   - [完整Chat流程](#完整chat流程)
   - [流式Chat流程](#流式chat流程)
   - [Skill注册流程](#skill注册流程)
   - [Provider切换流程](#provider切换流程)
6. [XML配置](#xml配置)
   - [aiagent_config.xml](#aiagent_configxml)
   - [moduleFactory_config.xml 中的注册](#modulefactory_configxml-中的注册)
7. [使用示例](#使用示例)
   - [示例1: 基本Chat调用](#示例1-基本chat调用)
   - [示例2: 多轮对话（带上下文）](#示例2-多轮对话带上下文)
   - [示例3: Tool Call自动触发](#示例3-tool-call自动触发)
   - [示例4: 流式Chat](#示例4-流式chat)
   - [示例5: 运行时注册自定义Skill](#示例5-运行时注册自定义skill)
   - [示例6: 管理上下文和记忆](#示例6-管理上下文和记忆)
   - [示例7: 运行时切换Provider](#示例7-运行时切换provider)
   - [示例8: 异步调用](#示例8-异步调用)
8. [扩展开发](#扩展开发)
   - [添加自定义LLM Provider](#添加自定义llm-provider)
   - [添加自定义Skill](#添加自定义skill)
   - [添加自定义Memory Store](#添加自定义memory-store)
9. [构建与运行](#构建与运行)
10. [Demo：一个可直接运行的多 Agent 应用](#demo一个可直接运行的多-agent-应用)
    - [一、跑起来](#一跑起来)
    - [二、命令行界面](#二命令行界面)
    - [三、Web 界面](#三web-界面)
...
Agent 世界继承父框架的哲学：**每个组件都是独立、自由、平等的对象**——由统一工厂创建、经消息互相交流。因此这里没有"胶水代码"，Agent 的每一个环节（思考、工具、记忆、编排）都是可被观察、拦截、替换的消息。

### 一、消息架构血统带来的结构性优势

- **全链路可观测、可插桩**：LLM 调用、工具执行、记忆存取全部走消息，`beforeMsgTable` / `afterMsgTable` 可在任意环节注入横切逻辑（记忆召回、审批、审计、统计）；`roundId` 贯穿全链，`/trace` 可对每一轮做定点重放分析
- **零特例组件**：Provider / Skill / Memory / 子 Agent / Agent 组 / 工作流，全部是 `TLBaseModule`——生命周期、热加载、配置方式与框架其它模块完全一致，学一套会全部
- **声明式装配**：System Prompt、工具清单、记忆策略、子 Agent 拓扑全部写在 XML 里；换模型（DeepSeek ⇄ OpenAI ⇄ Claude）、换记忆存储（文件 ⇄ 数据库）只改配置不改代码

### 二、对话智能

- **流式/非流式一套代码**：`chat()` 统一入口，SSE 流式输出（DeepSeek / OpenAI / Claude 兼容 Provider 即插即用）
- **模型路由 + 意图缓存**：简单问答自动走轻量模型省 Token，复杂任务自动升级强模型；高频意图学习后缓存命中秒回
- **推理链（ReAct）四模式**：`off / prompt / native / auto`——原生解析 DeepSeek `reasoning_content` 与 Claude Extended Thinking
- **上下文自动管理**：多轮裁剪、会话历史自动保存，长对话不失控

### 三、工具（Skill）能力

- **四类工具一体化**：内置 Skill（HTTP / 文件 / 代码执行 / 浏览器 / 桌面 GUI）+ 自定义 Skill + MCP 工具市场（一条命令安装）+ msgTool 消息工具（让 LLM 直接调用框架内任意消息路由）
- **并行工具执行**：LLM 一次返回多个 tool_calls 时并发执行、结果按序归位
- **热插拔**：Skill / 子 Agent 运行期注册、注销、热加载（`/install` `/reload`）
- **工具即 Agent**：子 Agent、Agent 组在工作流里与普通工具地位平等，master 零特判

### 四、三层记忆体系

| 层 | 载体 | 用途 |
|----|------|------|
| 会话上下文 | `TLAiContext` | 当轮对话连续性，自动裁剪 |
| 短期记忆 | 进程内 TTL 缓存 | 给外部 Skill 使用的临时状态 |
| 长期记忆 | 文件 JSONL / 数据库 | 跨会话知识碎片，摘要分层（碎片→摘要）+ 可选向量召回 |

记忆注入走 **AOP 消息钩子**：聊天前自动召回 → 注入 System 上下文 → 聊完后自动沉淀——业务代码零侵入，且天然跟随会话/用户隔离。

### 五、多 Agent 编排

- **子 Agent 委托**：描述路由自动分派，主控 Agent 不写任何类型特判
- **Agent 组**：串行链 / 并行合并 / 监理（supervisor）审核汇总
- **工作流 DAG + 表达式 DSL**：`&&` 并行、`\|\|` 回退、`->` 顺序、`if` 值分支——把编排写成一行表达式
- **断点续跑 / 直出**：中途可停可续（`/resume`），子 Agent 结果可绕过 LLM 加工直接返回（directOutput）

### 六、工程化与安全

- **HITL 审批门禁**：危险工具（文件删除、代码执行）按函数名审批，会话级拒绝记忆（同一操作不再重复打扰），审批框与 UI 完全解耦（事件总线推送）
- **确定性自测**：Mock Provider 驱动的 12 个测试场景，无网络、无 Key、结果可复现——改代码不怕回归
- **Evals 评测体系**：JSON 用例 + 三类 Judge（精确 / LLM / 约束），可批量回归
- **会话体系**：多用户数据隔离、断点续传、Token 统计、`/trace` 全链追踪
- **Web 交互界面（webui）**：浏览器聊天 + 截图可视化 + 会话管理，与框架同源零额外依赖

> 从"调用一次 LLM"到"企业级多 Agent 编排"，是同一条渐进的路：Mock 先跑通 → 换真实 Key 上线 → 按需加工具 → 再上编排与审批。每一步都不推翻前一步。
...
合并点的指认方式两种模式不同（策略写法完全一样）：

| 模式 | 合并点怎么来 | 怎么指认 |
|------|-------------|---------|
| 表达式 | `&&` / `\|\|` / `if` 自动生成 | `(A && B) as m1` 起名 → `merge.m1` |
| 静态节点 | 手写 `<node id="merge" type="join"/>` | **id 即名字** → `merge.merge` |

静态节点模式不需要 `as`（那是表达式模式的语法，因为编译器生成的 `__e1` 没法指认）；
不写 `merge.*` 也不会告警——那里不存在"起了名却没配"这回事。
...
**同一个字段名，各上游节点应返回同类型的值**——集合对集合、标量对标量。
`adaptive` 正是建立在这个前提上：同类型集合追加、同类型标量覆盖。

类型不一致（如一个上游返回 `"timeout"`、另一个返回 `["a","b"]`）时，合并结果由
**先到的那个值的类型**决定——同一对类型换个到达顺序结果就不同（标量→列表是覆盖、
列表→标量是追加）。这属于**上游违反了契约**，引擎会在工作流结束后打 WARN
（含字段名与两种类型），但**不改变合并行为**：契约应当去上游节点修，
不要指望靠合并策略兜——那只会把上游的问题掩盖起来。
