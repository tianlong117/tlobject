...
0. [Highlights: Why This Agent Framework](#highlights-why-this-agent-framework)
   - [1. Structural Advantages of the Message Architecture](#1-structural-advantages-of-the-message-architecture)
   - [2. Conversational Intelligence](#2-conversational-intelligence)
   - [3. Tool (Skill) Capabilities](#3-tool-skill-capabilities)
   - [4. Three-Layer Memory System](#4-three-layer-memory-system)
   - [5. Multi-Agent Orchestration](#5-multi-agent-orchestration)
   - [6. Engineering and Security](#6-engineering-and-security)
...
10. [Demo: A Runnable Multi-Agent Application](#demo-a-runnable-multi-agent-application)
    - [1. Run It](#1-run-it)
...
11. [Class Relationship Overview](#class-relationship-overview)
12. [New Feature Modules](#new-feature-modules)
...
How you address the merge point differs between the two modes (the strategy syntax is identical):

| Mode | Where the merge point comes from | How to address it |
|------|-------------|---------|
| Expression | auto-generated by `&&` / `\|\|` / `if` | name it with `(A && B) as m1` → `merge.m1` |
| Static nodes | a hand-written `<node id="merge" type="join"/>` | **the id *is* the name** → `merge.merge` |

Static mode needs no `as` (that is expression-mode syntax — a compiler-generated `__e1` cannot be addressed);
and omitting `merge.*` there raises no warning, since there is no such thing as a "named but unconfigured" point.
