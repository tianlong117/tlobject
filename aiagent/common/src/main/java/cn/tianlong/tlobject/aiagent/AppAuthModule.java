 ...
 * </ul>
 */
public class AppAuthModule extends TLBaseModule {

    private String userModule;
    private String authModule = "auth";

    public AppAuthModule() { super(); }
    public AppAuthModule(String name) { super(name); }
    public AppAuthModule(String name, TLObjectFactory modulefactory) { super(name, modulefactory); }
    ...
    private TLMsg doAuthForBatch(Object fromWho, TLMsg msg) {
        ...
        while (it.hasNext()) {
            ...
            if (task.module == null || task.moduleName == null || task.moduleName.isEmpty())
                continue;
            String userId = task.userId != null ? task.userId : "default";
