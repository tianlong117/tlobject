...
**解决的问题**：基线对比回答了"新挂了哪几条"，但"能不能上"还是靠人看。门禁把结论交给机器判。

启用（`evals_config.xml`，默认不启用）：
...
判定 = **通过率达标 且 回归数未超限**。两条同时管的意义在于：即使总体通过率仍达标，**新出现一条回归也算不通过**。

**flaky 用例不计入任何一边**（通过率分母与回归计数都排除 `metadata.stability != "stable"` 的用例）——一条时好时坏的用例就能让门禁随机变红，久了就没人看它了。

判定结果以 `gatePassed` / `gateEnabled` / `gateFailures` 返回，**不改 `RESULT` 语义**：`RESULT` 是"这个动作是否执行成功"，把门禁结果塞进去会让控制台把"评测跑完了但没过"误报成"评测失败"。控制台会多打一行（只在配了门禁时打）：

```
门禁: 不通过
  - 通过率 57.1% < 90.0%（stable 用例 4/7）
  - 出现 1 项回归（上限 0）: math-simple-001
```

两条容易踩的：

- **"没得比"不是"没有回归"**：要查回归但没有可比基准（首次运行、目录被清空）时，门禁判**不通过**并说明原因——否则它会看到空回归列表就放行，等于没查。建立基准后再跑一次即可。
- 启用前先让套件达到目标通过率。门禁一直红和一直绿一样没用，区别只是红的那种你会很快开始无视它。

### 4.5 无控制台入口（EvalGateCli）

`demo` 模块下的 `EvalGateCli` 让你不启动控制台就能跑完整检查，用退出码表达结果：
...
顺序是**先测试层（mock、免费、秒级）再评测层（真实 LLM、花钱）**：管线断了就没必要花 token 跑质量评测。1 与 2 分开是刻意的——脚本需要区分"代码坏了"和"质量退化了"。

`--timeout` 是**整条命令的总预算**（评测层卡住时兜底强制退出）；关闭阶段另有 60 秒独立预算，超时用已判定的退出码。

**门禁没配时它返回 2 而不是 0**：门禁未配置时 `gatePassed` 恒为 true，只读它的话用例全挂也会退 0——一个本该说真话的工具在说谎。

`-d` 要给**存在的目录**，而且要给**绝对路径**（例如 `D:/tlobjectapp/tlobject/demo/tlobject/target/classes/conf/demo/aiagent/`）。给相对路径会启动失败：`TLAppStartUp.startup()` 解析相对路径时配的默认目录是 `null`，会被拼成 `nulldemo/...`，紧接着抛 `StackOverflowError`（报错里那句 `realConfigDir : nulldemo/...` 就是它）。

`-d` 指的是**构建输出**里的配置目录（`target/classes/...`），改 `src/main/resources` 下的配置后要重新构建才会生效。

启动脚本（含机器路径）不入库，见 `_kept_local/`。

### 4.6 自动生成用例（/eval gen）

**解决的问题**：用例得手写，而写用例的成本高到会让人放弃做评测——这正是"不敢改"的源头之一。

```
/eval gen <agent> [要求描述]
/eval gen aiagent_master:priceTeam 重点测价格计算的准确性和需求含糊时是否追问
```

要求可省略；省略时纯按该 agent 的 md 自动推断该测什么。

**流程**：读该 agent 的 md 全文 + 枚举它的子模块 → LLM 出「能力点清单」（会在控制台打出来）→ LLM 按清单出用例 JSON → 程序化校验 → 落盘到 `evalCaseDir` → **只跑这一批**。
...
2. **只有真无法程序化的质量要求才用 `llm_judge`**，且 `prompt` 必须写成**逐条可核对的评分点**，禁止"回答是否准确友好"这类套话
3. **每条用 `llm_judge` 的用例必须同时带一条客观下限判据**——裁判放水时还有硬判据兜底
4. **覆盖三类**：正常路径 / 边界 / **不该做的事**

> 第 3、4 条不是洁癖：用 LLM 生成的判据去让 LLM 打分，很容易产出"全绿但没意义"的用例——跑一万次都绿，等你真改坏了它还是绿。那不叫护栏，叫安慰剂。

**丢弃的条目会说明原因**：缺 id、缺 judges、id 冲突的用例直接丢掉并在回执里列出——没有判据的用例会被评测**直接判通过**，那是假绿灯，宁可不要。
